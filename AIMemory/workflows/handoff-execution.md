...
## Handoff = 合约（铁律——超出合约的内容不能写）

**用户确认 handoff → 合约生效 → 严格按合约写代码。**

写代码时只做两类事：
- **合约里写了** → 直接实现
- **合约里没写、但代码需要**（新参数、新逻辑、新数值、新标签、新函数）→ **停下来，回来问用户**

不需要判断"这是 bug 还是机制"。只需要问自己一个问题：
...
**红线：绝不因为"代码写到一半卡住了"而自己补逻辑。卡住 = 合约不够详细 = 应该回来讨论。**
