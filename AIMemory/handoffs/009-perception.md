# Handoff 009 — 感知系统（四通道直推需求）

## 架构计划

**改什么：** 新建 `src/perception/mod.rs` + `src/perception/senses.rs`（2 文件）
**依据：** 四通道感知设计、PLOS Biology 2019 贝叶斯因果推理、Game Creator 2 Awareness
...
pub struct PerceptionResult {
    ...
    pub need_contributions: Vec<(NeedKind, f32)>, // (需求类型, 急迫度增量)
}
...
pub fn perceive_vision(
    ...
    observer_tags: &crate::tags::TagBits,  // 是否有 keen_eyed 等
    target_id: EntityId,
    target_pos: (u8, u8),
    target_tags: &crate::tags::TagBits,    // 是否有 hidden/camouflage 等
    target_size: u8,
    ...
    // 3. 被锁定——隐藏/伪装
    let hidden = target_tags.has(/* HIDDEN bit */);
    if hidden {
    ...
    if target_tags.has(/* PREDATOR bit */) {
        contributions.push((NeedKind::Safety, confidence * 0.8));
    }
    if target_tags.has(/* PREY/HERBIVORE bit */) {
...
pub fn perceive_hearing(
    ...
    if effective_volume < 0.1 { return None; }
    ...
    Some(PerceptionResult {
        detected_entity: EntityId(0), // 声音可能不对应具体 entity
        need_contributions: contributions,
    })
}
...
pub fn perceive_smell(
    ...
    let wind_factor = if dot > 0.0 { 1.0 + dot } else { (1.0 + dot).max(0.1) };
    
    let effective = odor_intensity * wind_factor * (1.0 - distance as f32 / observer_smell_range as f32);
    if effective < 0.05 { return None; }
...
**同构验证：**
- 四通道各自独立 → 不合并为觉察度 → 脑的多假设模型 ✅
- 感知直接推需求 → 不经过中间翻译层 → 叠加域原则 ✅
- 距离/光照/噪音/风向调制 → 物理环境影响感知 ✅

**性能：**
- 视觉：200 实体 × 15 邻居 × 1 次距离+遮挡检查 = 3000 次
- 听觉：按声音事件触发，偶发
- 嗅觉：200 实体 × 15 邻居 × 风向修正 = 3000 次
- 总计：< 0.5ms/tick
