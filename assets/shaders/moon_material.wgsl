// Low-Poly 月亮材质 — Flat Shading + 坑洼质感 + 微光
...
@group(#{MATERIAL_BIND_GROUP}) @binding(0) var<uniform> material: MoonMaterial;

// Simple 3D noise for crater-like bumps
...
fn fragment(
    ...
    // 混合基底色和坑色
    let base = mix(
        material.crater_color.rgb,
        material.base_color.rgb,
        smoothstep(0.3, 0.7, craters),
    );
